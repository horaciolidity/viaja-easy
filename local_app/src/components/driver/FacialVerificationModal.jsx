...
const FacialVerificationModal = ({ isOpen, onClose, request }) => {
  const videoRef = useRef(null);
  const [status, setStatus] = useState('idle'); // idle, loading_models, camera_permission, ready, verifying, success, error
  const [errorMessage, setErrorMessage] = useState('');
  const { toast } = useToast();
  const { runVerification } = useFaceAuth();
  ...
  const handleVerification = async () => {
    setStatus('verifying');
    try {
      const result = await runVerification(videoRef.current, request);
      if (result.success) {
        setStatus('success');
  ...
  const statusMessages = {
    ...
    ready: 'Mantené tu rostro dentro del recuadro.',
  ...
  const statusIcons = {
    ...
    verifying: <Loader2 className="w-16 h-16 animate-spin text-blue-500" />,
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[480px] bg-white dark:bg-gray-900 rounded-lg shadow-xl">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold text-center text-gray-800 dark:text-gray-100">Verificación Facial</DialogTitle>
        ...
        <div className="my-6 flex justify-center items-center">
          <div className="relative w-full max-w-sm h-64 bg-gray-200 dark:bg-gray-800 rounded-lg overflow-hidden flex justify-center items-center">
            <video ref={videoRef} autoPlay playsInline muted className={`w-full h-full object-cover transform -scale-x-100 ${status === 'ready' || status === 'verifying' ? 'opacity-100' : 'opacity-0'}`} />
            <div className={`absolute inset-0 flex flex-col justify-center items-center bg-black bg-opacity-50 transition-opacity ${status === 'ready' || status === 'verifying' ? 'opacity-0' : 'opacity-100'}`}>
              {statusIcons[status]}
            </div>
          </div>
        </div>
        ...
        <DialogFooter>
          {status === 'ready' && (
            <Button onClick={handleVerification} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 text-lg">
