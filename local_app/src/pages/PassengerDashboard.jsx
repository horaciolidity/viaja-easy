import React, { useState } from 'react';
    ...
    import { Car, CalendarDays, Timer, MapPin, PackagePlus, Search, Users, Route, Compass, ArrowRight, AlertCircle } from 'lucide-react';
    ...
    const PassengerDashboard = () => {
      ...
      return (
        <div className="space-y-8">
          ...
          {!isVerified && (
            <div className="p-4 bg-yellow-100 border-l-4 border-yellow-500 text-yellow-800 rounded-lg flex items-center justify-between">
          ...
          <section>
            <h2 className="text-xl font-semibold mb-4 text-slate-800 dark:text-slate-100">Planificá tu Viaje</h2>
    ...
    const BannerCard = ({ title, description, icon: Icon, onClick, bgColor, disabled }) => (
      <motion.div 
        className={`relative p-6 rounded-2xl overflow-hidden text-white shadow-lg ${bgColor} ${disabled ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer'}`}
      ...
      >
        <div className="relative z-10">
          ...
          <Button variant="secondary" className="mt-4 bg-white/20 hover:bg-white/30 text-white rounded-full" disabled={disabled}>
    ...
    const MainServiceCard = ({ icon: Icon, title, description, onClick, highlight, disabled }) => (
      <motion.div
        onClick={disabled ? undefined : onClick}
        className={`p-6 rounded-2xl shadow-lg transition-all transform ${disabled ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer hover:-translate-y-1.5'} ${highlight ? 'shadow-xl' : 'bg-white dark:bg-slate-800'}`}
        style={{ background: highlight ? 'linear-gradient(145deg, hsl(220, 80%, 60%) 0%, hsl(240, 90%, 70%) 100%)' : '' }}
        whileHover={!disabled ? { scale: 1.03 } : {}}
        whileTap={!disabled ? { scale: 0.98 } : {}}
      >
        <div className={`w-14 h-14 mb-4 rounded-xl flex items-center justify-center shadow-md ${highlight ? 'bg-white/20' : 'bg-primary dark:bg-sky-500'}`}>
          <Icon className="w-7 h-7 text-white" />
        </div>
        <h3 className={`text-lg font-semibold mb-1 ${highlight ? 'text-white' : 'text-slate-800 dark:text-slate-100'}`}>{title}</h3>
        <p className={`text-xs ${highlight ? 'text-white/90' : 'text-slate-500 dark:text-slate-400'}`}>{description}</p>
      </motion.div>
    );
